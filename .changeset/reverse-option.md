...
- **`CIRCLE` / `FAN` / `RIPPLE` / `CLOCK_SWEEP` / `CURTAIN` 五个生效**（依次：圆面向内收拢 / 扇叶合拢 / 水面内收 / **逆时针扫开** / 两扇幕布向中线合拢），其余类型传了静默忽略、非法值静默回落 `false`。`RIPPLE` / `CLOCK_SWEEP` 的接入排在后续版本；形状族与 `CURTAIN` 经实测判定**做不到无副作用**、不接入（理由见下）。
