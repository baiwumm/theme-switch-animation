...
<template>
  <main>
    ...
    <p>
      ...
      <code>observeThemeClass</code>），其它标签页的切换经 storage 事件同步。中心扩散与角度扫开类动画（含 RIPPLE / CLOCK_SWEEP / FAN）的起收点都是按钮中心：在不同位置点击可验证跟随效果；BLINDS / SCAN / QR_GRID / CURTAIN 不读触发元素几何。前一组卡片下方各有独立的 direction 选择，RIPPLE 另有 waveWidth 档位、FAN 另有 bladeCount 档位，都只影响本卡片。
    </p>
