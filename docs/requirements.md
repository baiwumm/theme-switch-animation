...
| `reverse` | `boolean \| 'auto'` | `false` | 反向揭开：`true` 恒反向、`'auto'` 切暗正向 / 切亮收起。与 `direction` 正交（后者定推进轴、前者定从内还是从外揭开）。当前仅 `CIRCLE` 生效，其余类型静默忽略；非法值静默回落 `false`（v1.10 新增） |
