...
- 🎨 **动画类型分族**：圆形扩散（含收起 / 模糊变体）、几何形状扩散、条带与格子（由 `direction` 控制四方向，含奇偶错拍的梳齿交错）、中线对开、环带前缘（由 `waveWidth` 控制波长）、角度扫开（由 `bladeCount` 控制扇叶数）。完整清单与逐个的观感说明见下方「🎬 动画类型」表
...
| `COMB` | 梳齿交错：奇数叶片先展开、偶数叶片错半拍跟上，两层渐变取并集 | 无触发点（全屏按 2×叶宽平铺） | `direction` / `slatWidth` |

`BLINDS` / `SCAN` / `QR_GRID` / `CURTAIN` / `COMB` 是属性驱动蒙版（`@property --theme-switch-reveal` + 静止蒙版盒子），不读触发元素几何——`ref` 只用于点击与状态。`RIPPLE` / `CLOCK_SWEEP` / `FAN` 用同一机制但把轴心写进渐变串，因此消费 `ref` 几何；角度族另用 `--theme-switch-sweep`（`@property` 的 syntax 一经注册不可改，`<angle>` 不能与 `<length>` 同名）。`QR_GRID` 的"列 ∩ 行"双层蒙版交集经 `@supports (mask-composite: intersect)` 门控，不支持的引擎自动降级为推进轴单层条带（观感同百叶窗），状态始终正确。
...
| `direction` | `'ltr' \| 'rtl' \| 'ttb' \| 'btt'` | 扫描方向，默认 `ltr`。仅 `BLINDS` / `SCAN` / `QR_GRID` / `COMB` 生效（可用 `ThemeAnimationDirection` 常量），非法值静默回落默认 |
| `slatWidth` | `number` | 百叶窗叶片宽度 px，范围 `[16, 200]`，默认 72。仅 `BLINDS` / `COMB` 生效，越界静默回落默认 |
